...
export class EditCardComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.previewcard$ = this.quizzForm.valueChanges.pipe(
      map(formValues=>
        this.card = new QuizzCard(this.card.id ,formValues.domaine , formValues.difficulte , formValues.question , formValues.reponse, formValues.explication, this.card.publication,this.card.date,this.card.userID)
      )
    )
  ...
  onSubmitForm(){
    this.QuizzCardService.putCard(this.getCardJson(this.card)).subscribe(message =>{this.router.navigateByUrl('/mycards')});
  };
